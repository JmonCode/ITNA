...
- `SUPABASE_SERVICE_ROLE_KEY`가 필요하다. 슈퍼어드민 bootstrap, 관리자 전용 데이터 처리, 태그 upsert처럼 RLS를 우회해야 하는 서버 전용 작업에서만 사용한다.
- `ADMIN_EMAILS=smily094@gmail.com`을 `.env.local`과 Vercel 환경변수에 모두 설정한다.
- `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`가 없으면 앱은 데모/설정 안내 상태로 동작한다.
